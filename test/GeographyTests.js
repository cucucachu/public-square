  ...
  before(function (done) {
    User.clear().then(function () {
      GeographicArea.clear().then(function () {
        GeographicMap.clear().then(function () {
          MapType.clear().then(function () {
            Address.clear().then(function () {
  ...
  describe('GeographicArea Model Tests', function () {
    describe('GeographicArea.create()', function () {
      it('GeographicArea.create() creates a GeographicArea instance.', function () {
        ...
        assert(_typeof(geographicArea) === "object");
      });
      it('GeographicArea.create() creates a GeographicArea instance with _id field populated', function () {
        var geographicArea = GeographicArea.create();
        assert(_typeof(geographicArea._id) === "object" && /^[a-f\d]{24}$/i.test(geographicArea._id));
      });
    });
    describe('GeographicArea.save()', function () {
      it('GeographicArea.save() throws an error if required fields are missing.', function (done) {
        ...
        GeographicArea.save(geographicArea).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('GeographicArea.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('GeographicArea.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
  ...
  describe('GeographicMap Model Tests', function () {
    describe('GeographicMap.create()', function () {
      it('GeographicMap.create() creates a GeographicMap instance.', function () {
        ...
        assert(_typeof(geographicMap) === "object");
      });
      it('GeographicMap.create() creates a GeographicMap instance with _id field populated', function () {
        var geographicMap = GeographicMap.create();
        assert(_typeof(geographicMap._id) === "object" && /^[a-f\d]{24}$/i.test(geographicMap._id));
      });
    });
    describe('GeographicMap.save()', function () {
      it('GeographicMap.save() throws an error if required fields are missing.', function (done) {
        ...
        GeographicMap.save(geographicMap).then(function (result) {
          ...
          if (testFailed) done(new Error('GeographicMap.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('GeographicMap.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
  ...
  describe('MapType Model Tests', function () {
    describe('MapType.create()', function () {
      it('MapType.create() creates a MapType instance.', function () {
        ...
        assert(_typeof(mapType) === "object");
      });
      it('MapType.create() creates a MapType instance with _id field populated', function () {
        var mapType = MapType.create();
        assert(_typeof(mapType._id) === "object" && /^[a-f\d]{24}$/i.test(mapType._id));
      });
    });
    describe('MapType.save()', function () {
      it('MapType.save() throws an error if required fields are missing.', function (done) {
        ...
        MapType.save(mapType).then(function (result) {
          ...
          if (testFailed) done(new Error('MapType.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('MapType.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
  ...
  describe('Address Model Tests', function () {
    describe('Address.create()', function () {
      it('Address.create() creates a Address instance.', function () {
        ...
        assert(_typeof(address) === "object");
      });
      it('Address.create() creates a Address instance with _id field populated', function () {
        var address = Address.create();
        assert(_typeof(address._id) === "object" && /^[a-f\d]{24}$/i.test(address._id));
      });
    });
    describe('Address.save()', function () {
      it('Address.save() throws an error if required fields are missing.', function (done) {
        ...
        Address.save(address).then(function (result) {
          ...
          if (testFailed) done(new Error('Address.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('Address.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
