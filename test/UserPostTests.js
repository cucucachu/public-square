...
require('../dist/models/Modules/UserPost/UserPostModule'); // Add 'finally()' to 'Promise.prototype'
  ...
  describe('UserPost Model', function () {
    describe('UserPost.create()', function () {
      it('create() creates a UserPost instance.', function () {
        ...
        assert(_typeof(userPost) === "object");
      });
      it('create() creates a UserPost instance with _id field populated', function () {
        var userPost = UserPost.create();
        assert(_typeof(userPost._id) === "object" && /^[a-f\d]{24}$/i.test(userPost._id));
      });
    });
    describe('UserPost.save()', function () {
      it('UserPost.save() throws an error if required fields are missing.', function (done) {
        ...
        UserPost.save(userPost).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('UserPost.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('UserPost.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('UserPost.poster must be a valid ID', function (done) {
        ...
        UserPost.save(userPost).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          err = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves UserPost.', function (done) {
        ...
        userPost.stamps = [Stamp.create()._id, Stamp.create()._id];
        UserPost.save(userPost).then(function (saved) {
          UserPost.findById(userPost._id).then(function (found) {
  ...
  describe('Poster Model', function () {
    describe('Poster.create()', function () {
      it('create() creates a Poster instance.', function () {
        ...
        assert(_typeof(poster) === "object");
      });
      it('create() creates a Poster instance with _id field populated', function () {
        var poster = Poster.create();
        assert(_typeof(poster._id) === "object" && /^[a-f\d]{24}$/i.test(poster._id));
      });
    });
    describe('Poster.save()', function () {
      it('Poster.save() throws an error if required fields are missing.', function (done) {
        ...
        Poster.save(poster).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('Poster.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('Poster.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('Poster.userAccount must be a valid ID', function (done) {
        ...
        Poster.save(poster).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves Poster.', function (done) {
        ...
        poster.userPosts = [UserPost.create()._id, UserPost.create()._id];
        Poster.save(poster).then(function (saved) {
          Poster.findById(poster._id).then(function (found) {
  ...
  describe('Stamp Model', function () {
    describe('Stamp.create()', function () {
      it('create() creates a Stamp instance.', function () {
        ...
        assert(_typeof(stamp) === "object");
      });
      it('create() creates a Stamp instance with _id field populated', function () {
        var stamp = Stamp.create();
        assert(_typeof(stamp._id) === "object" && /^[a-f\d]{24}$/i.test(stamp._id));
      });
    });
    describe('Stamp.save()', function () {
      it('Stamp.save() throws an error if required fields are missing.', function (done) {
        ...
        Stamp.save(stamp).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('Stamp.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('Stamp.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('Stamp.stamper must be a valid ID', function (done) {
        ...
        Stamp.save(stamp).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves Stamp.', function (done) {
        ...
        stamp.stampType = ApprovalStampType.create()._id;
        Stamp.save(stamp).then(function (saved) {
          Stamp.findById(stamp._id).then(function (found) {
  ...
  describe('Stamper Model', function () {
    describe('Stamper.create()', function () {
      it('create() creates a Stamper instance.', function () {
        ...
        assert(_typeof(stamper) === "object");
      });
      it('create() creates a Stamper instance with _id field populated', function () {
        var stamper = Stamper.create();
        assert(_typeof(stamper._id) === "object" && /^[a-f\d]{24}$/i.test(stamper._id));
      });
    });
    describe('Stamper.save()', function () {
      it('Stamper.save() throws an error if required fields are missing.', function (done) {
        ...
        Stamper.save(stamper).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('Stamper.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('Stamper.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('Stamper.stamps must be a valid Array of IDs', function (done) {
        ...
        Stamper.save(stamper).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves Stamper.', function (done) {
        ...
        stamper.stamps = [Stamp.create()._id];
        Stamper.save(stamper).then(function (saved) {
          Stamper.findById(stamper._id).then(function (found) {
  ...
  describe('ApprovalStampType Model', function () {
    describe('ApprovalStampType.create()', function () {
      it('create() creates a ApprovalStampType instance.', function () {
        ...
        assert(_typeof(approvalStampType) === "object");
      });
      it('create() creates a ApprovalStampType instance with _id field populated', function () {
        var approvalStampType = ApprovalStampType.create();
        assert(_typeof(approvalStampType._id) === "object" && /^[a-f\d]{24}$/i.test(approvalStampType._id));
      });
    });
    describe('ApprovalStampType.save()', function () {
      it('ApprovalStampType.save() throws an error if required fields are missing.', function (done) {
        ...
        ApprovalStampType.save(approvalStampType).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('ApprovalStampType.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('ApprovalStampType.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('ApprovalStampType.weight must be a number.', function (done) {
        ...
        ApprovalStampType.save(approvalStampType).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves ApprovalStampType.', function (done) {
        ...
        ApprovalStampType.save(approvalStampType).then(function (saved) {
          ApprovalStampType.findById(approvalStampType._id).then(function (found) {
  ...
  describe('ObjectionStampType Model', function () {
    describe('ObjectionStampType.create()', function () {
      it('create() creates a ObjectionStampType instance.', function () {
        ...
        assert(_typeof(objectionStampType) === "object");
      });
      it('create() creates a ObjectionStampType instance with _id field populated', function () {
        var objectionStampType = ObjectionStampType.create();
        assert(_typeof(objectionStampType._id) === "object" && /^[a-f\d]{24}$/i.test(objectionStampType._id));
      });
    });
    describe('ObjectionStampType.save()', function () {
      it('ObjectionStampType.save() throws an error if required fields are missing.', function (done) {
        ...
        ObjectionStampType.save(objectionStampType).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('ObjectionStampType.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('ObjectionStampType.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('ObjectionStampType.weight must be a number.', function (done) {
        ...
        ObjectionStampType.save(objectionStampType).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves ObjectionStampType.', function (done) {
        ...
        ObjectionStampType.save(objectionStampType).then(function (saved) {
          ObjectionStampType.findById(objectionStampType._id).then(function (found) {
  ...
  describe('PostStream Model', function () {
    describe('PostStream.create()', function () {
      it('create() creates a PostStream instance.', function () {
        ...
        assert(_typeof(postStream) === "object");
      });
      it('create() creates a PostStream instance with _id field populated', function () {
        var postStream = PostStream.create();
        assert(_typeof(postStream._id) === "object" && /^[a-f\d]{24}$/i.test(postStream._id));
      });
    });
    describe('PostStream.save()', function () {
      it('PostStream.save() throws an error if required fields are missing.', function (done) {
        ...
        PostStream.save(postStream).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('PostStream.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('PostStream.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('PostStream.userGroup must be a valid ObjectId.', function (done) {
        ...
        PostStream.save(postStream).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves PostStream.', function (done) {
        ...
        postStream.userPosts = [UserPost.create()._id];
        PostStream.save(postStream).then(function (saved) {
          PostStream.findById(postStream._id).then(function (found) {
  ...
  describe('ExternalLink Model', function () {
    describe('ExternalLink.create()', function () {
      it('create() creates a ExternalLink instance.', function () {
        ...
        assert(_typeof(externalLink) === "object");
      });
      it('create() creates a ExternalLink instance with _id field populated', function () {
        var externalLink = ExternalLink.create();
        assert(_typeof(externalLink._id) === "object" && /^[a-f\d]{24}$/i.test(externalLink._id));
      });
    });
    describe('ExternalLink.save()', function () {
      it('ExternalLink.save() throws an error if required fields are missing.', function (done) {
        ...
        ExternalLink.save(externalLink).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('ExternalLink.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('ExternalLink.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('ExternalLink.userPosts must be a valid Array of IDs.', function (done) {
        ...
        ExternalLink.save(externalLink).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves ExternalLink.', function (done) {
        ...
        ExternalLink.save(externalLink).then(function (saved) {
          ExternalLink.findById(externalLink._id).then(function (found) {
  ...
  describe('ArticleLink Model', function () {
    describe('ArticleLink.create()', function () {
      it('create() creates a ArticleLink instance.', function () {
        ...
        assert(_typeof(articleLink) === "object");
      });
      it('create() creates a ArticleLink instance with _id field populated', function () {
        var articleLink = ArticleLink.create();
        assert(_typeof(articleLink._id) === "object" && /^[a-f\d]{24}$/i.test(articleLink._id));
      });
    });
    describe('ArticleLink.save()', function () {
      it('ArticleLink.save() throws an error if required fields are missing.', function (done) {
        ...
        ArticleLink.save(articleLink).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('ArticleLink.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('ArticleLink.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('ArticleLink.userPosts must be a valid Array of IDs.', function (done) {
        ...
        ArticleLink.save(articleLink).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves ArticleLink.', function (done) {
        ...
        ArticleLink.save(articleLink).then(function (saved) {
          ArticleLink.findById(articleLink._id).then(function (found) {
  ...
  describe('ImageLink Model', function () {
    describe('ImageLink.create()', function () {
      it('create() creates a ImageLink instance.', function () {
        ...
        assert(_typeof(imageLink) === "object");
      });
      it('create() creates a ImageLink instance with _id field populated', function () {
        var imageLink = ImageLink.create();
        assert(_typeof(imageLink._id) === "object" && /^[a-f\d]{24}$/i.test(imageLink._id));
      });
    });
    describe('ImageLink.save()', function () {
      it('ImageLink.save() throws an error if required fields are missing.', function (done) {
        ...
        ImageLink.save(imageLink).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('ImageLink.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('ImageLink.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('ImageLink.userPosts must be a valid Array of IDs.', function (done) {
        ...
        ImageLink.save(imageLink).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves ImageLink.', function (done) {
        ...
        ImageLink.save(imageLink).then(function (saved) {
          ImageLink.findById(imageLink._id).then(function (found) {
  ...
  describe('VideoLink Model', function () {
    describe('VideoLink.create()', function () {
      it('create() creates a VideoLink instance.', function () {
        ...
        assert(_typeof(videoLink) === "object");
      });
      it('create() creates a VideoLink instance with _id field populated', function () {
        var videoLink = VideoLink.create();
        assert(_typeof(videoLink._id) === "object" && /^[a-f\d]{24}$/i.test(videoLink._id));
      });
    });
    describe('VideoLink.save()', function () {
      it('VideoLink.save() throws an error if required fields are missing.', function (done) {
        ...
        VideoLink.save(videoLink).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          error = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('VideoLink.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (error != null && error.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('VideoLink.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + error.message));
            }
          }
        });
      });
      it('VideoLink.userPosts must be a valid Array of IDs.', function (done) {
        ...
        VideoLink.save(videoLink).then(function (saved) {
          testFailed = 1;
        }, function (saveErr) {
          error = saveErr;
        }).finally(function () {
      ...
      it('Valid Call Saves VideoLink.', function (done) {
        ...
        VideoLink.save(videoLink).then(function (saved) {
          VideoLink.findById(videoLink._id).then(function (found) {
