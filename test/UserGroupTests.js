    ...
    User.clear().then(function () {
      UserAccount.clear().then(function () {
        UserRole.clear().then(function () {
          GroupManager.clear().then(function () {
            GroupMember.clear().then(function () {
  ...
  describe('UserGroup Model', function () {
    ...
    describe('UserGroup.addChildren()', function () {
      it('UserGroup.addChildren() rejects with an error if children parameter is not an Array.', function (done) {
        ...
        UserGroup.addChildren(parentGroup, childGroup).then(function () {
          testFailed = 1;
      ...
      it('UserGroup.addChildren() rejects with an error if children already have a different parent.', function (done) {
        ...
        UserGroup.addChildren(parentGroup, childGroups).then(function () {
          testFailed = 1;
          error = new Error('UserGroup.save() promise resolved when it should have rejected with an error.');
        }, function (saveError) {
        ...
        UserGroup.addChildren(parentGroup, childGroups).then(function () {
          UserGroup.addChildren(parentGroup, childGroups).then(function () {
            UserGroup.Model.findById(parentGroup._id, function (findError, found) {
              if (found.childGroups.length != 2) error = new Error('Parent UserGroup should have 2 children, but it has ' + found.childGroups.length + '.');
            });
          }, function (saveError) {
            testFailed = 1;
            error = saveError;
        ...
        UserGroup.addChildren(parentGroup, childGroups).then(function () {
          UserGroup.Model.findById(parentGroup._id, function (findErr, found) {
            found.childGroups.forEach(function (child, index) {
              if (child != childGroups[index]._id) error = new Error('Children do not match');
            });
          });
        }, function (saveErr) {
          testFailed = 1;
