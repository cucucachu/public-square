...
require('../dist/models/Modules/User/UserModule'); // Add 'finally()' to 'Promis.prototype'
  ...
  describe('User Role Model', function () {
    describe('UserRole.create()', function () {
      it('create() creates a userRole instance', function () {
        ...
        assert(_typeof(userRole) === "object");
      });
      it('create() creates a userRole instance with _id field populated', function () {
        var userRole = UserRole.create();
        assert(_typeof(userRole._id) === "object" && /^[a-f\d]{24}$/i.test(userRole._id));
      });
    });
    describe('UserRole.save()', function () {
      it('Required fields validation', function (done) {
        ...
        UserRole.save(userRole).then(function (result) {
          testFailed = 1;
        }, function (rejectionErr) {
          err = rejectionErr;
        }).finally(function () {
          if (testFailed) done(new Error('UserRole.save() promise resolved when it should have been rejected with Validation Error'));else {
            if (err != null && err.message == expectedErrorMessage) {
              done();
            } else {
              done(new Error('UserRole.save() did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + err.message));
            }
          }
        });
      });
      it('UserRole.User() must be a valid ID', function (done) {
        ...
        UserRole.save(userRole).then(function (savedUserRole) {
          testFailed = 1;
        }, function (saveErr) {
          err = saveErr;
        }).finally(function () {
          if (testFailed) {
            ...
            if (err != null && err.message == expectedErrorMessage) {
              ...
              done(new Error('UserRole.saveUserRole did not return the correct Validation Error.\n' + '   Expected: ' + expectedErrorMessage + '\n' + '   Actual:   ' + err.message));
            }
          }
        });
      });
      it('Valid call saves userRole', function (done) {
        ...
        UserRole.save(userRole).then(function (savedUserRole) {
          UserRole.Model.findOne({
            _id: savedUserRole._id
          }, function (findErr, foundUserRole) {
            if (findErr) {
              err = findErr;
            } else {
    ...
    describe('Creating and Saving Users, UserAccounts, and UserRoles', function () {
      ...
      describe('UserAccount.saveUserAndUserAccount()', function () {
        it('UserAccount.saveUserAndUserAccount() saves both instances.', function (done) {
          ...
          userAccount.email = 'email@domain.com';
          userAccount.passwordHash = 'aasdf;lkjwoiethoinwaf;f;vno32890y4r8qhpajr98etj8tntaijffijfa';
          UserAccount.saveUserAndUserAccount(user, userAccount).then(function () {
      ...
      describe('User.addUserRoletoUser()', function () {
        it('Adds user Role to existing User with no current Roles', function (done) {
          ...
          UserAccount.saveUserAndUserAccount(user, userAccount).then(function () {
            ...
            User.addUserRoletoUser(user, userRole).then(function () {
